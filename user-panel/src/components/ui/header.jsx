...
import { setHeaderHeight, setIsModelOpen, setIsMobileMenuOpen } from "@/redux/slices/commonSlice";
...
const Header = () => {
  ...
  const { collection, categories, subCategories, isModelOpen, isMobileMenuOpen, headerHeight } = useSelector(
    (state) => state.common
  );
    ...
    <>
      ...
      >
        <Link className="cursor-pointer" href={"/"}>
          <CustomImage srcAttr={logo} altAttr={"logo"} titleAttr={"logo"} className="w-40" />
        </Link>
        ...
        <section className="flex gap-6 items-center">
          <div className="group">
            <h2 className={`${hoverClassname} text-sm lg:text-[17px]`}>Become a Seller</h2>
          ...
          <div className="flex gap-4 items-center">
            ...
            <RiUser3Line onClick={() => dispatch(setIsModelOpen(true))} className={`${hoverClassname} w-full h-5`} />
          </div>
        </section>
      </header>
      ...
      <header className="flex lg:hidden w-full px-4 py-3 bg-white justify-between items-center border-b border-gray-100 sticky top-0 z-40">
        <div className="flex items-center">
          {isMobileMenuOpen ? (
            <HiX className="w-6 h-6 cursor-pointer" onClick={() => dispatch(setIsMobileMenuOpen(false))} />
          ) : (
            <HiOutlineMenuAlt3 className="w-6 h-6 cursor-pointer" onClick={() => dispatch(setIsMobileMenuOpen(true))} />
          )}
        </div>

        <div className="absolute left-1/2 -translate-x-1/2">
          <Link href={"/"}>
            <CustomImage srcAttr={logo} altAttr={"logo"} titleAttr={"logo"} className="w-28" />
          </Link>
        </div>

        <div className="flex gap-4 items-center">
          <HiMiniMagnifyingGlass className="w-5 h-5 text-gray-700 cursor-pointer" />
          <RiUser3Line onClick={() => dispatch(setIsModelOpen(true))} className="w-5 h-5 text-gray-700 cursor-pointer" />
                ...
                <div key={item._id} className="py-3">
                  <div className="flex justify-between items-center text-[15px] font-medium text-gray-800 uppercase tracking-wide">
                    ...
                    {dynamicCategories.length > 0 && (
                      <span onClick={() => toggleCollection(item._id)} className="p-2 cursor-pointer">
                        {openCollectionId === item._id ? <FaChevronUp className="w-3 h-3 text-gray-500" /> : <FaChevronDown className="w-3 h-3 text-gray-500" />}
                      </span>
                    )}
                  </div>

                  {/* LEVEL 2: CATEGORIES */}
                  {openCollectionId === item._id && dynamicCategories.length > 0 && (
                    <div className="pl-3 mt-2 flex flex-col gap-1 border-l-2 border-primary/20">
                      {dynamicCategories.map((cat) => {
                        ...
                        const visibleSubs = isExpanded ? dynamicSubs : dynamicSubs.slice(0, 5);
                          ...
                          <div key={cat._id} className="py-2">
                            <div className="flex justify-between items-center text-sm font-medium text-gray-700">
                              ...
                              {dynamicSubs.length > 0 && (
                                <span onClick={() => toggleCategory(cat._id)} className="p-2 cursor-pointer">
                                  {openCategoryId === cat._id ? <FaChevronUp className="w-2.5 h-2.5 text-gray-400" /> : <FaChevronDown className="w-2.5 h-2.5 text-gray-400" />}
                                </span>
                              )}
                            </div>

                            {/* LEVEL 3: SUB-CATEGORIES */}
                            {openCategoryId === cat._id && dynamicSubs.length > 0 && (
                              <div className="pl-4 mt-2 text-xs text-gray-500 space-y-2.5 bg-gray-50/70 p-2 rounded">
                                
                                {dynamicSubs.length > 5 && (
                                  <button 
                                    onClick={(e) => handleViewAllSubCategories(e, cat._id)}
      ...
      {/* GLOBAL AUTH MODAL */}
      {isModelOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm" onClick={() => dispatch(setIsModelOpen(false))}>
